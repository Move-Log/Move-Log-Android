...
import com.ilgusu.presentation.databinding.ItemMonthBinding
import java.util.Calendar
import java.util.Date

class NewsMonthAdapter(
    private var currentMonthOffset: Int,
    private var selectedDate: Date?,
) : RecyclerView.Adapter<NewsMonthAdapter.MonthView>() {
    ...
    override fun onBindViewHolder(holder: MonthView, position: Int) {
        ...
        for (i in 0 until 6 * 7) {
            dayList[i] = calendar.time
            calendar.add(Calendar.DAY_OF_MONTH, 1)
        }


        val dayListManager = GridLayoutManager(holder.binding.root.context, 7)
        val dayListAdapter = NewsDayAdapter(currentMonth, dayList) { selectedDate ->
